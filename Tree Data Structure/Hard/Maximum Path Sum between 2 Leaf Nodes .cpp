...
int dummy(Node *root,int &ans)
{
    ...
    //maximum sum in left and right sub tree
    int left=dummy(root->left,ans);
    int right=dummy(root->right,ans);
    
    if(root->left!=NULL && root->right!=NULL)
    {
    //cal. maximum sum of tree
    ans=max(ans,left+right+root->data);
    //here returning max sum from root to leaf 
